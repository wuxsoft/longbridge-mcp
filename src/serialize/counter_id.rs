...
use crate::serialize::{FieldKind, Transformed, delegate_simple};

pub(crate) struct CounterIdValue<'a, T: ?Sized> {
    pub(crate) value: &'a T,
}
...
pub(crate) struct CounterIdsValue<'a, T: ?Sized> {
    pub(crate) value: &'a T,
}
...
pub(crate) struct CounterIdsSeq<S> {
    inner: S,
}
